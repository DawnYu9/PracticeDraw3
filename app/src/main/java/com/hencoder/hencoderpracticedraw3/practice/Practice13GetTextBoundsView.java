...
public class Practice13GetTextBoundsView extends View {
...

    {
        ...
        for (int i = 0; i < 6; i++) {
            paint2.getTextBounds(texts[i], 0, texts[i].length(), bounds);
            yOffset[i] = -(bounds.top + bounds.bottom) / 2;
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 6; i++) {
            canvas.drawText(texts[i], 100 + i * 100, middle + yOffset[i], paint2);
        }
